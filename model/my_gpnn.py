...
class gpnn:
    ...
    def PNN_faiss(self, x, x_scaled, y_scaled, patch_size, stride, alpha, mask=None, mask_key = None,mask_query = None,
        ...
        # import ipdb;ipdb.set_trace = lambda :None; print('setting set_trace to None')
        ...
        assert y_scaled[0].shape[-1] in [3,4]
        queries = torch.stack([extract_patches(ys, patch_size, stride) for ys in y_scaled],dim=0)
        ...
        if False and mask is not None:
            ...
            # import ipdb;ipdb.set_trace()
            if True:
                queries = queries[:,mask[:,0]]
        elif mask_query is not None:
        ...
        if queries_flat_batch.shape[0] == 0 :
            ...
            '''
            assert False,'this is not tested for mask_key and mak_query'
            # import ipdb;ipdb.set_trace()
            assert mask.ndim == 2
            assert mask.shape[-1] == 1
            print('TODO:should this be x or x_scaled? (x_scaled will be blurry)')
            patch_aggregation_results = dict(
                I = torch.arange(mask.shape[0],device=device)[:,None],
                D = torch.zeros(mask.shape[0],device=device),
                combined = x,
                patch_aggregation = self.PATCH_AGGREGATION,
            )
            print('early exit as no queries')
            import pdb;pdb.set_trace()
            return patch_aggregation_results
            '''
        ...
        if new_keys:
            ...
            if self.TASK == 'inpainting':
                if False:
                    ...
                    assert mask.ndim == 2
                    assert mask.shape[-1] == 1
                    all_I = torch.arange(mask.shape[0],device=device)
                    # import pdb;ipdb.set_trace()
                    assert mask.dtype == torch.bool
                    holes_at = mask[:,0]
                    keys_to_keep = all_I[~holes_at]
                    assert (keys[keys_to_keep].abs().sum(dim=(1,2,3)) == 0).sum() == 0
                else:
            ...
            #import ipdb;ipdb.set_trace()
            # keys_to_keep = [1,2];print('hard coding keys to keep as [1,2]')
            print('Check keys to keep here')
            #import ipdb;ipdb.set_trace()
            # keys_for_index[keys_to_keep]
            '''
            if (keys_to_keep is not None) and (len(keys_to_keep) > 200): 
                self.index = create_index(keys_for_index,index_type='ivf',index_options={'nlist':200},keys_to_keep=keys_to_keep)
            else:
                self.index = create_index(keys_for_index,index_type='simple')
            '''
            #==================================================================
            # if index_type == 'ivf':
            #     self.index = create_index(keys_for_index,index_type='ivf',index_options={'nlist':200},keys_to_keep=keys_to_keep)           
        ...
        '''
        subset_ids = [1,2,3]
        selector = faiss.IDSelectorArray((len(subset_ids)),np.array(subset_ids).astype(np.int64))
        '''
        ...
        if self.TASK == 'inpainting':
            if False:
                ...
                I[mask.squeeze(1)] = partial_I
                D[mask.squeeze(1)] = partial_D
            else:
                print('reinsert the partial I back into fullI')
                partial_I,partial_D = I,D
                I = torch.arange(mask_query.shape[0],device=device)[:,None]
                ...
                assert D.shape[-1] == 1
                D = torch.zeros(I.shape,device=device).float()
                I[mask_query.squeeze(1)] = partial_I
                D[mask_query.squeeze(1)] = partial_D                
        print(queries_proj.shape)
        if self.TASK == 'inpainting':
            if False:
                assert mask.shape[0] == I.shape[0], 'partial_I should have been reinserted within I'
            else:
        ...
        if False and mask is not None:
            ...
            # we dont select from only the unmasked values as 
            # keys_to_keep will not select any values from the hole.

            if False:
                print('TODO:is the following statement true?')
                assert not mask[I.squeeze()].any(),'I should not be indexing into any true region of the mask (true regions are the ones with holes)'
                values[mask.squeeze(1)] = values[I.squeeze(1)]
        ...
        print('combined')
        if patch_aggregation_results['combined'].shape[-1] not in [3,4]:
            import pdb;pdb.set_trace()
