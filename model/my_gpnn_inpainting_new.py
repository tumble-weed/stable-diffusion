...
class gpnn:
    def __init__(self, config):
        ...
        if config['task'] == 'inpainting':
            ...
            def create_pyramid2(mask_tensor,R,PATCH_SIZE,STRIDE):
                ...
                #=================================================
                from pyrdown import masked_filter2d
                device = mask_tensor.device
                kernel = torch.tensor([[[0,0.25,0],[0.25,0,0.25],[0,0.25,0]]],device=device)
                #=================================================
                last_x = self.input_img_tensor
                x_pyramid = []
                complement_mask = (1 - mask_tensor)
                last_mask_complement = complement_mask

                while True:
                    if True:
                        x_filled = masked_filter2d(
                            last_x,
                            kernel,
                            last_mask_complement,
                            border_type = 'reflect',
                        )
                         
                        complement_mask_filled = masked_filter2d(
                            last_mask_complement,
                            kernel,
                            last_mask_complement,
                            border_type = 'reflect',
                        ...
                        #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
                        x_filled = x_filled * (1 - last_mask_complement) + last_x * last_mask_complement
                        complement_mask_filled = torch.maximum(complement_mask_filled,last_mask_complement)
                        #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
                    else:
                        x_filled = last_x
                        complement_mask_filled = last_mask_complement
                    
                    assert ((complement_mask_filled - last_mask_complement) >= 0).all()
                    assert complement_mask_filled.unique().shape[0] <= 2
                    # assert complement_mask_filled.shape[-2:] == x_filled.shape[-2:]
                    if i == 1:
                        complement_smaller_bchw = pyrdown(
                        complement_mask_filled, 
                        mask = complement_mask_filled,
                        border_type = 'reflect', 
                        align_corners = True, 
                        factor = R)
                    else:
                        _, _, height, width = complement_mask_filled.shape
                        complement_smaller_bchw = torch.nn.functional.interpolate(
                            complement_mask_filled,
                            size=(int(float(height) / R), int(float(width) // R)),
                            mode='bilinear',
                            align_corners=True,
                        )
                    ...
                    if i == 1:
                        x_smaller_bchw = pyrdown(x_filled, 
                                    mask = complement_mask_filled,
                                    border_type = 'reflect', 
                                    align_corners = True, 
                                    factor = self.R)
                    else:
                        _, _, height, width = x_filled.shape
                        x_smaller_bchw = torch.nn.functional.interpolate(
                            x_filled,
                            size=(int(float(height) / R), int(float(width) // R)),
                            mode='bilinear',
                            align_corners=True,
                        )
                    if False and 'binarize':
                        ...
                        denom = ( complement_smaller_bchw + (complement_smaller_bchw == 0.).float())
                        x_smaller_bchw = x_smaller_bchw/ denom
                        complement_smaller_bchw = complement_smaller_bchw / denom
                        complement_smaller = complement_smaller_bchw.permute(0,2,3,1)
                        smaller = 1 - complement_smaller
                    '''
                    ...
                    # x_pyramid.append(x_filled)

                    # assert x_smaller_bchw.shape[-2:] == complement_smaller_bchw.shape[-2:]
                    x_smaller = x_smaller_bchw.permute(0,2,3,1)
                    x_pyramid.append(x_smaller)
                    last_x = x_smaller_bchw
                    last_mask_complement = complement_smaller_bchw
                    last_mask_complement = (last_mask_complement == 1.).float()
                    if len(x_pyramid) == 15:
                        import ipdb;ipdb.set_trace()
                    if smaller.sum() == 0:
                        break
                ...
                # import ipdb;ipdb.set_trace()
                #=================================================
                if False and 'len problem':
                    assert len(x_pyramid) == len(mask_pyramid)
                mask_pyramid.insert(0,mask_tensor.permute(0,2,3,1))
                x_pyramid.insert(0,(self.input_img_tensor* (1- mask_tensor)).permute(0,2,3,1))
                '''
                ...
                if False and 'len problem':
                    assert len(x_pyramid) == len(mask_pyramid)
                # assert len(x_pyramid) == len(query_pyramid)
    ...
    def run(self, to_save=True):
        ...
        for i in tqdm.tqdm(reversed(range(len(self.x_pyramid)))):
            ...
            #=====================================================
            if (~self.key_pyramid[i]).sum() == 0:
                # import ipdb;ipdb.set_trace()
                if i == len(self.x_pyramid) - 1:
                    self.y_pyramid[i] = self.x_pyramid[i]
                else:
                    upscaled = torch.nn.functional.interpolate(self.y_pyramid[i+1].permute(0,3,1,2),self.x_pyramid[i].shape[1:3]).permute(0,2,3,1)
                    self.y_pyramid[i] = self.x_pyramid[i] * (1 - self.mask_pyramid[i]) +  upscaled * self.mask_pyramid[i]
                continue
            #=====================================================            
            
            assert len(self.x_pyramid) == len(self.mask_pyramid)
            assert len(self.x_pyramid) == len(self.query_pyramid)
