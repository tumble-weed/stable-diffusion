'''
contains some copied code from my_gpnn_inpainting_new. ( does not include creating pyramid till the smallest level.) ( includes rescaling the x_pyramid values using the mask_pyramid)
'''
import os
import tqdm
import numpy as np
...
from pyrdown import pyrdown, _get_pyramid_gaussian_kernel, masked_filter2d, gaussian_kernel
...
class gpnn:
    def __init__(self, config):
        ...
        # self.PATCH_SIZE = (3,3);print(colorful.gold(f'setting patch size {self.PATCH_SIZE}'))
        ...
        self.PATCH_AGGREGATION = 'distance-weighted'
        print(colorful.orange3(f'{self.PATCH_AGGREGATION}'))
        self.INDEX_TYPE = config['index_type']
        if os.environ.get('DBG_USE_XY',None) is not None:
            config['use_xy'] = os.environ['DBG_USE_XY']
            config['use_xy'] = (config['use_xy'] == '1')
        self.USE_XY = config['use_xy']
        # self.USE_XY = False; print(colorful.turquoise(f'USE_XY is set to {self.USE_XY}'))
        # cuda init
        global device
        ...
        self.ORIGINAL_SIZE = self.input_img_tensor.shape[-2:]
        # pyramids
        #=======================================================================
        def upsize_tensors(img,mask_tensor):
                complement_mask_tensor  = 1 - mask_tensor
                ...
                up_img = torch.nn.functional.interpolate(img,size=(img.shape[-2]*UPSIZE_FACTOR,img.shape[-1]*UPSIZE_FACTOR),mode='nearest')
                up_complement = torch.nn.functional.interpolate(complement_mask_tensor,size=(img.shape[-2]*UPSIZE_FACTOR,img.shape[-1]*UPSIZE_FACTOR),mode='nearest')
                up_mask = 1- up_complement
                # assert False
                up_img = up_img/ (up_mask + (up_mask == 0.).float())
                up_mask = up_mask / (up_mask + (up_mask == 0.).float())
        ...
        '''         
        UPSIZE = True
        if os.environ.get('DBG_NO_UPSIZE',None) == '1':
            UPSIZE = False
        if UPSIZE:
            original_input_img_tensor,original_mask_tensor = self.input_img_tensor,self.mask_tensor

            self.input_img_tensor,self.mask_tensor = upsize_tensors(self.input_img_tensor,
                                                   self.mask_tensor) 
            self.input_img = tensor_to_numpy(self.input_img_tensor.permute(0,2,3,1))[0]
            print(self.input_img_tensor.shape,self.mask_tensor.shape)
            self.COARSE_DIM = original_input_img_tensor.shape[-2:]
            # print(self.input_img_tensor.max())
            # assert False                            
        '''
        #=======================================================================
        ...
        if config['task'] == 'inpainting':
            ...
            def create_pyramid2(mask_tensor,R,PATCH_SIZE,STRIDE,min_size=None):
                mask_pyramid = []
                x_pyramid = []
                i = 1
                query_pyramid = []
                ...
                #=================================================
                while True:
                    complement_smaller_bchw = pyrdown(
                        (1 - mask_tensor), 
                        mask = (1-mask_tensor),
                        border_type = 'reflect', 
                        align_corners = True, 
                        factor = R ** i)
                    if 'x_smaller':
                        #TODO: take input_img_tensor as argument 
                        assert mask_tensor.shape[1] == 1
                        assert mask_tensor.ndim == 4
                        x_smaller_bchw = pyrdown(self.input_img_tensor, 
                                    mask = torch.tensor( (1 - mask_tensor),device=device),
                                    border_type = 'reflect', 
                                    align_corners = True, 
                                    factor = R ** i)
                        x_smaller_bchw = x_smaller_bchw/(
                            complement_smaller_bchw + (complement_smaller_bchw == 0.).float()
                        )
                    if 'insert mask smaller into pyramid':
                        #complement_smaller = complement_smaller_bchw.permute(0,2,3,1)
                        smaller_bchw = 1 - complement_smaller_bchw
                        #smaller = (smaller != 0.).float();print(colorful.yellow('setting the pyramid level to binary eary'))
                        smaller_bchw = smaller_bchw/(smaller_bchw + (smaller_bchw==0.).float())
                        smaller = smaller_bchw.permute(0,2,3,1)
                    ...
                    x_smaller = x_smaller_bchw.permute(0,2,3,1)# permute now or later
                    x_pyramid.append(x_smaller)
                    #====================================================
                    # # standard pyramid:
                ...
                print('check input_img_tensor.shape')
                #import pudb;pudb.set_trace()
                # print(self.input_img_tensor.max())
                # assert False
                x_pyramid.insert(0,self.input_img_tensor.permute(0,2,3,1))
                # print(colorful.red())
                # print(colorful.red('TODO:cutting off mask_pyramid at 8'))

                if min_size:
                    # import ipdb; ipdb.set_trace()
                    cutoff_lvl = None
                    for ilvl,lvl in enumerate(mask_pyramid):
                        if min(lvl.shape[1:3]) < min(min_size):
                            cutoff_lvl = ilvl
                            break
                    mask_pyramid = mask_pyramid[:cutoff_lvl]
                    print(colorful.red(f'TODO:cutting off mask_pyramid at {cutoff_lvl}'))
                elif False:
                    cutoff = 8
                    mask_pyramid = mask_pyramid[:cutoff]
                    print(colorful.red(f'TODO:cutting off mask_pyramid at {cutoff}'))
                ...
                # import ipdb; ipdb.set_trace()
                for ii in reversed(range(n_levels)):
                    if any([query_pyramid[ii].all(),key_pyramid[ii].all()]):
                        continue
                    break
                mask_pyramid,query_pyramid,key_pyramid,x_pyramid = mask_pyramid[:ii+1],query_pyramid[:ii+1],key_pyramid[:ii+1],x_pyramid[:ii+1]
                return mask_pyramid,query_pyramid,key_pyramid,x_pyramid
            
            self.mask_pyramid,self.query_pyramid,self.key_pyramid,self.x_pyramid = create_pyramid2(self.mask_tensor,self.R,self.PATCH_SIZE,self.STRIDE,
                                                                                                #    min_size = self.ORIGINAL_SIZE
     min_size= None
    )
            # import ipdb;ipdb.set_trace()
            # print(self.x_pyramid[0].max())
            # assert False
        ...
        if self.USE_XY:
            for i_x,x in enumerate(self.x_pyramid):
                Y,X = torch.meshgrid(torch.linspace(0,1,x.shape[1],device=device),
                               ...
                               indexing = 'ij')
                self.spatial_mag = 0.3
                x = torch.cat([x,self.spatial_mag*Y[None,:,:,None],self.spatial_mag*X[None,:,:,None]],dim=-1)
                self.x_pyramid[i_x] = x
                # import ipdb;ipdb.set_trace()
        else:
            print(colorful.turquoise('not using xy'))
        #============================================================
        # import pdb;pdb.set_trace()
        ...
        if config['task'] == 'inpainting':
            ...
            elif False and 'cut-off zero key':
                mask_pyramid.cutoff_pyramid_at_zero([~l for l in self.key_pyramid],self.key_pyramid,self.x_pyramid,self.query_pyramid)        
            elif True:
                self.y_pyramid,self.key_pyramid,self.x_pyramid,self.query_pyramid = self.y_pyramid[:pyramid_depth],self.key_pyramid[:pyramid_depth],self.x_pyramid[:pyramid_depth],self.query_pyramid[:pyramid_depth]
            # import ipdb;ipdb.set_trace()
            if False:
        ...
        if False and 'limit_pyramids':
            till = 3
            print(colorful.red(f'limiting pyramids to {till}'))
    ...
    def run(self, to_save=True):
        ...
        for i in tqdm.tqdm(reversed(range(len(self.x_pyramid)))):
            ...
            if i == len(self.x_pyramid) - 1:
                ...
                # import ipdb;ipdb.set_trace()
                if False and 'blurring the first set of keys':
                    # import pyrdown
                    # gkernel = _get_pyramid_gaussian_kernel()
                    gkernel = gaussian_kernel(11,5)
                    x_bchw = self.x_pyramid[i].permute(0,3,1,2)
                    mask_bchw = (1 - self.mask_pyramid[i]).permute(0,3,1,2)
                    to_blur = x_bchw
                    for kk in range(20):
                        to_blur = masked_filter2d(to_blur, gkernel,mask_bchw, 'reflect')
                    keys_blur_bchw = to_blur
                    keys_blur = keys_blur_bchw.permute(0,2,3,1)
                    dutils.img_save(tensor_to_numpy(keys_blur)[0,...,:3],'x_pyramid1.png')
                    # import ipdb;ipdb.set_trace()
                    keys = keys_blur
            else:
                ...
                if False and 'adding per level noise':
                    ...
                    if not os.environ.get('DBG_IGNORE_RESIZE',False) == '1':
                        assert False,'all resize_bhwc should be with masked_conv?'
            ...
            if i == len(self.x_pyramid) - 1:
                if self.query_pyramid[i].any():
                    ...
                    if os.environ.get('DBG_NAIVE',False) == '1':
                        imputed = (im_for_road * mask_for_road).unsqueeze(0).permute(0,2,3,1).to(device)
                ...
                # self.T = 1;print(colorful.red(f"setting self.T to {self.T}"))
                for j in tqdm.tqdm(range(self.T)):                
                    if self.is_faiss:
                        ...
                        if self.USE_XY :
                            ...
                            if (
                                True and
                                i == (len(self.x_pyramid) - 2)
                                # True
                                and (j < 1)
                                ):
                                Y,X = torch.meshgrid(torch.linspace(0,1,queries.shape[1],device=device),
                                            torch.linspace(0,1,queries.shape[2],device=device),
                                            indexing = 'ij')
                                # assert False
                                
                                new_queries = torch.cat([queries[...,:3],new_spatial_mag*Y[None,:,:,None],new_spatial_mag*X[None,:,:,None]],dim=-1)
                                queries = new_queries
                                # import ipdb;ipdb.set_trace() 
                                if False:
                                    keys[...,3:] /= spatial_mag
                                    keys[...,3:] *= new_spatial_mag
                                else:
                                    keys = torch.cat([keys[...,:3],new_spatial_mag*Y[None,:,:,None],new_spatial_mag*X[None,:,:,None]],dim=-1)
                            elif (i == (len(self.x_pyramid) - 3) and (j < 1)) :
                                queries[...,3:]/=new_spatial_mag
                                queries[...,3:]*= self.spatial_mag
                            # elif self.USE_XY:
                            #     Y,X = torch.meshgrid(torch.linspace(0,1,queries.shape[1],device=device),
                            #                 torch.linspace(0,1,queries.shape[2],device=device),
                            #                 indexing = 'ij')
                            ...
                            # import ipdb;ipdb.set_trace()
                            # elif i < (len(self.x_pyramid) - 2):
                            #     print(colorful.red("ignoring xy for higher pyramid levels"))
                            #     queries[...,3:] =  0
                            #     keys[...,3:] = 0
                            #==============================================================
        ...
        for iq,_ in enumerate(self.query_pyramid):
            ...
            # patch_aggregation = 'distance-weighted'
        ...
        #================================================================
        if os.environ.get('DBG_SAVE_IMAGES',False) == '1':
            dutils.img_save(tensor_to_numpy(self.y_pyramid[-1][...,:3])[0],'y_pyramid_last.png')
            dutils.img_save(tensor_to_numpy(self.y_pyramid[-2][...,:3])[0],'y_pyramid_2.png')
            dutils.img_save(tensor_to_numpy(self.y_pyramid[last][...,:3])[0],'y_pyramid.png')
            dutils.img_save(tensor_to_numpy(self.mask_pyramid[last])[0],'mask_pyramid.png')
            dutils.img_save(tensor_to_numpy(self.x_pyramid[0][...,:3])[0],'x_pyramid.png')
        
        holefilled= self.y_pyramid[last].permute(0,3,1,2)
        holefilled = torch.nn.functional.interpolate(holefilled,size=self.ORIGINAL_SIZE,mode='bilinear')
        if os.environ.get('DBG_BREAK_NEAR_END',False) == '1':
            import ipdb;ipdb.set_trace()        

        return holefilled,resolution_results
    ...
    def PNN_faiss(self, x, x_scaled, y_scaled, patch_size, stride, alpha, mask=None, mask_key = None,mask_query = None,
        ...
        keys = extract_patches(x_scaled, patch_size, stride)
        if False:
            keys = torch.cat([keys,keys.flip((-2,))],dim=0)
            print(colorful.red("adding vertically flipped keys"))
        if False:
            small_patch_size= 3,3
            key_mask = torch.zeros(*keys.shape[-2:],device=device)
            key_mask[...,
                    patch_size[0]//2 - small_patch_size[0]//2:-(patch_size[1]//2 - small_patch_size[1]//2),
                    patch_size[0]//2 - small_patch_size[0]//2:-(patch_size[1]//2 - small_patch_size[1]//2)
                    ] = 1
            keys =keys*key_mask[None,None]
            print(colorful.red("multiplying keys by transparency mask"))
        # import ipdb;ipdb.set_trace()
        #====================================================
    ...
    def get_pca_feats(self,input,init=False,reproducible=False):
        ...
        if init:
            # import ipdb;ipdb.set_trace()
            self.N_PCA_COMPONENTS = min(self.N_PCA_COMPONENTS,np.prod(input.shape[1]))
