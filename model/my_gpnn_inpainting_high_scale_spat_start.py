...
from pyrdown import pyrdown, _get_pyramid_gaussian_kernel, masked_filter2d
    ...
    def run(self, to_save=True):
        ...
        for i in tqdm.tqdm(reversed(range(len(self.x_pyramid)))):
            ...
            if i == len(self.x_pyramid) - 1:
                ...
                queries = self.coarse_img.detach().clone()
                keys = self.x_pyramid[i].detach().clone()
                # import ipdb;ipdb.set_trace()
            ...
            if False:
                ...
                for j in tqdm.tqdm(range(self.T)):                
                    if self.is_faiss:
                        ...
                        #==============================================================
                        if (os.environ.get('DBG_NO_ROAD',False) == '1'): 
                            if (
                                i == (len(self.x_pyramid) - 1) and self.USE_XY 
                                and (j < 2)
                            ):
                                # import ipdb; ipdb.set_trace()
                                print(colorful.yellow("using just the spatial term for the first step"))
                                Y,X = torch.meshgrid(torch.linspace(0,1,queries.shape[1],device=device),
                                            torch.linspace(0,1,queries.shape[2],device=device),
                                            indexing = 'ij')
                                new_spatial_mag = 3
                                new_queries = torch.cat([torch.zeros_like(queries[...,:3]),new_spatial_mag*Y[None,:,:,None],new_spatial_mag*X[None,:,:,None]],dim=-1)
                                queries = new_queries
                                
                                if False:
                                    keys[...,:3] = torch.zeros_like(keys[...,:3])
                                    keys[...,3:] /= 0.3
                                    keys[...,3:] *= new_spatial_mag                            
                                else:
                                    new_keys1 = torch.cat([torch.zeros_like(keys[...,:3]),new_spatial_mag*Y[None,:,:,None],new_spatial_mag*X[None,:,:,None]],dim=-1)
                                    keys = new_keys1
                                # import ipdb; ipdb.set_trace()
                        elif (
