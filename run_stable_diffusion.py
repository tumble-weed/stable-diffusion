...
def place_in_large_image(orig_img_np,orig_mask_np):
    ...
    small_res_np = skimage.transform.resize(orig_img_np,(256,256))
    small_canvas_np = np.full((256,256,3), 1.)
    # small_mask_img_np = np.array(orig_mask_pil.resize((256,256))) # mask of the object
    small_mask_np = skimage.transform.resize(orig_mask_np,(256,256))
...
def run(impath='flask3.jpg'):
    ...
    # repasted_np = tensor_to_numpy(repasted)
    repasted_np = (repasted)
    extras = {'holefilled':holefilled_np}
    return gen_image_np,repasted_np,extras
