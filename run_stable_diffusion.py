...
def run_on_folder(folder,results_root='results'):
    ...
    for impath in glob.glob(os.path.join(folder,'*')):
        ...
        skimage.io.imsave(os.path.join(savefolder,'generated_all.png'),np_final)
        

        if os.environ.get('shifted_down',False) == '1':
            import sys
            sys.exit()
...
if __name__ == '__main__':
    ...
    if True and 'folder':
        ...
        #folder = "perfume"
        # folder = "cosmetics"
        run_on_folder(folder,results_root=f'results-{folder}')
    """
